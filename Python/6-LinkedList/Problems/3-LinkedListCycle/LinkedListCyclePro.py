...
def has_cycle_approach1(head):
    ...
    - Basic approach, good for understanding the problem.
    - Used in simple data validation scenarios.
...
def has_cycle_approach3(head):
    ...
    Approach 3: Modifying the linked list (adding a visited flag).
     - Modifies the original linked list, which might not be desirable in many real applications.
     - Could be used in a non-production, single-use script where modifying the list is acceptable.
    Time Complexity: O(n)
...
def has_cycle_approach4(head):
    ...
    - Useful if you need to know the number of times each node is visited, not just the existence of a cycle.
...
def has_cycle_approach5(head):
    """
    Approach 5: Using a long and short pointer with a counter.
    - Similar to Approach 2, but adds a counter to limit the number of steps.
    - Important in systems where infinite loops can cause crashes or resource exhaustion.
    - The max_count acts as a safety mechanism.
...
def has_cycle_approach6(head):
    """
    Approach 6: Using threads and timeouts for cycle detection.
    - This approach uses a separate thread to check for a cycle, with a timeout.
    - Useful in real-time systems or when dealing with potentially long-running linked list operations.
    - Prevents the main thread from being blocked indefinitely.

    Time Complexity:  O(n) in best case, but bounded by the timeout.
    Space Complexity: O(1)
    """
    if not head:
        return False

    def check_for_cycle(head, result):
        """
    ...
    result = {'cycle_found': False}
    thread = threading.Thread(target=check_for_cycle, args=(head, result))
    thread.start()
    thread.join(timeout=0.5)  # Set a timeout (adjust as needed)

    return result['cycle_found']
...
if __name__ == "__main__":
    ...
    test_cases = [
        ...
        ([1, 2, 3, 4, 5], 10), # pos greater than length of list.
    ]
    ...
    for nodes, cycle_pos in test_cases:
        ...
        print(f"  Approach 6 (Threads):         {'Cycle' if has_cycle_approach6(head) else 'No Cycle'}")
